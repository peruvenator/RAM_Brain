...
ax.plot(x_line, m_slope*x_line + m_int, color=C_SG, linewidth=1.5, linestyle='--', alpha=0.7,
        label=f'β = {m_slope:.2f}, α = {m_int:.2f}%')
...
SRC_LINE = f"Source: ReSolve Asset Management SEZC (Cayman) (replication model data), Société Générale (SG Trend Index data). Calculations by Newfound Research. Data from {start_str} through {end_str}."
...
def fig_block(num, title, img_key, description=""):
    ...
    parts = [f"Source: ReSolve Asset Management SEZC (Cayman) (replication model data), Société Générale (SG Trend Index data). Calculations by Newfound Research. Data from {start_str} through {end_str}."]
    if description:
        parts.append(description)
    parts.append("Trend Replication model returns are hypothetical, net of estimated transaction costs and management fees, and do not represent actual trading. The SG Trend Index is shown on an excess return basis and is net of underlying fund fees, which include management fees and trading costs and may include performance fees. Hypothetical performance results have many inherent limitations, some of which are described in the Important Disclosures section of this document. Past performance is not indicative of future results.")
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .header {{ text-align: center; margin-bottom: 28px; padding-bottom: 18px; border-bottom: 2px solid #0d9488; }}
  ...
  .section h2 {{ font-size: 14px; font-weight: 600; color: #0d9488; margin-bottom: 8px; padding-bottom: 4px; border-bottom: 1px solid #e2e8f0; }}
  ...
  .stat-card {{ background: #f8fafc; padding: 10px 12px; border-left: 3px solid #0d9488; }}
  ...
  .neg {{ color: #ef4444; }}
  .pos {{ color: #10b981; }}
  .callout {{ background: #f8fdfb; border-left: 3px solid #0d9488; padding: 8px 12px; margin: 12px 0; font-size: 10px; }}
...
<div class="header">
  <div style="font-size: 8px; color: #94a3b8; font-style: italic; margin-bottom: 8px;">For use with financial advisors only. Not for distribution to the general public.</div>
...
<div class="section">
  ...
  <p>The blend's hypothetical cumulative return of <strong>{stats['Blend']['cum_ret']:.1f}%</strong> compares to <strong>{sg_cum:.1f}%</strong> for the SG Trend Index over the same period, a gap of {stats['Blend']['cum_ret'] - sg_cum:.1f} percentage points. This difference, explored in detail below, reflects both the implementation cost drag inherent in any replication program and periods of model divergence. There is no guarantee that this level of tracking quality will persist in future periods.</p>
</div>
...
<div class="section">
  ...
  <div class="figure-disclosure">Source: ReSolve Asset Management SEZC (Cayman) (replication model data), Société Générale (SG Trend Index data). Calculations by Newfound Research. Data from {start_str} through {end_str}. Trend Replication model returns are hypothetical, net of estimated transaction costs and management fees, and do not represent actual trading. The SG Trend Index is shown on an excess return basis and is net of underlying fund fees, which include management fees and trading costs and may include performance fees. Hypothetical performance results have many inherent limitations, some of which are described in the Important Disclosures section of this document. Adjusted Tracking Error (Lo) applies the autocorrelation adjustment of Lo (2002) using a Bartlett kernel with {LO_Q} lags. Past performance is not indicative of future results. See Technical Glossary for metric definitions.</div>
</div>
...
<div class="section">
  ...
  {fig_block(5, "Lo-Adjusted Exponentially-Weighted Annualized Tracking Error vs. SG Trend Index (252-Day Half-Life)",
             "rolling_te",
...
<div class="section">
  ...
  <p>Historically, the blend ran ahead of the SG Trend Index for the first several months before gradually giving back relative performance. The cumulative gap appeared to stabilize in more recent quarters, though there is no assurance that this pattern will persist.</p>
</div>
...
<div class="section">
  ...
  <div class="two-col">
    <div>
      {fig_block(8, f"Monthly Return Scatter — Blend vs. SG Trend Index (ρ = {corr_m:.3f})",
                 "scatter_monthly",
                 f"Each dot represents one calendar month. The dashed line shows ordinary least squares regression (beta = {m_slope:.2f}, alpha = {m_int:.2f}%). The light gray line is the 45-degree reference.")}
    </div>
    <div>
      <h3>Interpretation</h3>
      <p style="margin-top: 24px;">The clustering around the 45-degree line suggests a strong historical monthly fit. The regression beta of <strong>{m_slope:.2f}</strong> and monthly correlation of <strong>{m_corr_blend:.3f}</strong> indicate that, over this period, the blend has generally captured the direction and magnitude of monthly SG Trend moves.</p>
  ...
  {fig_block(9, "Side-by-Side Monthly Returns — SG Trend Index vs. Trend Replication Blend",
             "monthly_bars",
             "Each pair of bars represents one calendar month. SG Trend Index is shown in black and the Trend Replication Blend in teal.")}
</div>
...
<div class="section">
  ...
  <p>The ex-post optimal weights are approximately <strong>{w_opt[0]:.0%} Top Down (Constrained) / {w_opt[1]:.0%} Top Down (Full) / {w_opt[2]:.0%} Bottom Up</strong>, which would have produced a Lo-adjusted annualized tracking error of <strong>{te_opt:.1f}%</strong> versus <strong>{te_current:.1f}%</strong> for the current blend — a reduction of roughly {te_current - te_opt:.1f} percentage points.</p>
...
html += f"""    </tbody>
  ...
  <div class="figure-disclosure">Source: ReSolve Asset Management SEZC (Cayman) (replication model data), Société Générale (SG Trend Index data). Calculations by Newfound Research. Data from {start_str} through {end_str}. Quarterly returns are calculated from the last trading day of the prior quarter to the last trading day of the displayed quarter. Trend Replication model returns are hypothetical, net of estimated transaction costs and management fees, and do not represent actual trading. The SG Trend Index is shown on an excess return basis and is net of underlying fund fees, which include management fees and trading costs and may include performance fees. Past performance is not indicative of future results.</div>
</div>
...
<div class="disclosure-section">
  ...
  <p>Replication model data is sourced from ReSolve Asset Management SEZC (Cayman). SG Trend Index data is sourced from Société Générale. All calculations are performed by Newfound Research. While data is believed to be reliable, Newfound Research does not guarantee its accuracy or completeness. Trend-following strategies, including replication programs, are subject to numerous risks including but not limited to market risk, leverage risk, liquidity risk, counterparty risk, model risk, and the risk that past trends may not persist. Replication programs face additional risks including signal lag, tracking error, divergence from the target index, and the risk that the target index methodology may change without notice. There is no guarantee that the Trend Replication Program will continue to achieve the level of tracking quality demonstrated during the historical period analyzed herein. Newfound Research LLC and/or its affiliates may have financial interests in products that utilize the Trend Replication models described herein. This analysis should not be considered independent research.</p>
</div>

<div class="footer">
  Analysis generated {datetime.now().strftime('%B %d, %Y')} · Data: {start_str} – {end_str}<br>
  © {datetime.now().year} Newfound Research LLC. All rights reserved.
</div>
...
print("✓ HTML report v2 generated successfully")
